...
@router.get("", tags=["Images CRUD endpoints"], response_model=List[Image])
async def get_images(
    request: Request,
    ownerId: str | None = Query(None, description="ID del autor de la imagen", gt=0),
...
@router.get("/{id}", tags=["Images CRUD endpoints"], response_model=Image)
...
@router.post("", tags=["Image Upload endpoint"])
async def test_upload(file: UploadFile = File(...)):
    try:
        ...
        new_image.ownerId = str(1 + (int)(10 * random.random()))
        new_image.url = thumbnail_url

        body_dict = new_image.model_dump()
        body_dict["date"] = datetime.now()

        DatabaseConnection.create_document("image", body_dict)
...
@router.options("", tags=["Images OPTIONS endpoints"])
...
@router.options("" + "/{id}", tags=["Images OPTIONS endpoints"])
...
def build_query(ownerId: Optional[str], name: Optional[str]) -> Dict[str, Dict]:
